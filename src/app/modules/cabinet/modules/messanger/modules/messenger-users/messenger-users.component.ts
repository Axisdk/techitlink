...
export class MessengerUsersComponent implements OnInit, OnDestroy {
	...
	@Output() companion: EventEmitter<Pick<UserInterface, 'id' | 'avatar_url' | 'fname' | 'lname'>> = new EventEmitter<
		Pick<UserInterface, 'id' | 'avatar_url' | 'fname' | 'lname'>
	>();
	...
	protected getFullNameCompanion(companion: Pick<UserInterface, 'id' | 'avatar_url' | 'fname' | 'lname'>): string {
		if (!companion) return '';
